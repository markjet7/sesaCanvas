...
class MainWindow(QMainWindow):
    ...
    def load_system(self, path=None):
        ...
        try:
            ...
            self.system_dropdown.clear()
            self.system_dropdown.addItems(["Systems..."] + self.system_names)
    ...
    def draw(self, system):
        ...
        graph, layout = layout_system(system)
        positions = {}
        grid_x = 250
        grid_y = 100
        ...
        def placeLine(stream):
            ...
            if stream.source and stream.sink:
                # self.scene.addLine(positions[stream.source.ID][0],
                #                    positions[stream.source.ID][1],
                #                    positions[stream.sink.ID][0],
                #                    positions[stream.sink.ID][1],
                #                    pen)
                
                arrow = Arrow(positions[stream.source.ID][0],
                                positions[stream.source.ID][1],
                                positions[stream.sink.ID][0],
                                positions[stream.sink.ID][1],
                ...
                # arrow.link_start(rects[stream.source.ID], self.scene) 
                # arrow.link_end(rects[stream.sink.ID], self.scene)
                text = self.scene.addText(stream.ID, textFont)
                text.setDefaultTextColor(Qt.GlobalColor.black)
                text.setPos(
                    (positions[stream.source.ID][0] + positions[stream.sink.ID][0])/2,
                    (positions[stream.source.ID][1] + positions[stream.sink.ID][1])/2)
            elif stream.source and stream.sink == None:
                # self.scene.addLine(positions[stream.source.ID][0],
                #                    positions[stream.source.ID][1],
                #                    positions[stream.ID][0],
                #                    positions[stream.ID][1],
                #                    pen)
                arrow = Arrow(positions[stream.source.ID][0],
                                positions[stream.source.ID][1],
                                positions[stream.ID][0],
                ...
                text.setDefaultTextColor(Qt.GlobalColor.black)
                text.setPos(
                    positions[stream.source.ID][0] + 50,
                    positions[stream.source.ID][1] - 25)
            elif stream.sink and stream.source == None:
                # self.scene.addLine(positions[stream.ID][0],
                #                    positions[stream.ID][1],
                #                    positions[stream.sink.ID][0],
                #                    positions[stream.sink.ID][1],
                #                    pen)
                arrow = Arrow(positions[stream.ID][0],
                                positions[stream.ID][1],
                                positions[stream.sink.ID][0],
                                positions[stream.sink.ID][1],
                                sink=rects[stream.sink.ID])
                ...
                text.setDefaultTextColor(Qt.GlobalColor.black)
                text.setPos(
                    (positions[stream.ID][0] - 50),
                    (positions[stream.ID][1] - 25))
        ...
        list(map(placeUnit, system.units))
        list(map(placeLine, system.streams))
